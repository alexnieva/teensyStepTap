...
int tapBPM = 120;     // calculated tap BPM. Value to be defined by Condition 2.
int walkBPM = 96;    // calculated cadence. Value to be defined by Condition 1.
...
//          tap_offset_threshold   = 500;
        ...
        // Here we need to calculate walkBPM
        prev_t = current_t;
        if (current_t - trialStartTime >= trialInterval+100){
          // Here we calculate walkBPM.
//          Serial.println(responseArray[msg_number_array][2]);
//          Serial.println(responseArray[1][2]);
//          Serial.println(msg_number_array);
          walkBPM = 2*round(60E3/((responseArray[msg_number_array][2] - responseArray[1][2])/(msg_number_array - 1)))-1;
          Serial.print("walkBPM: "); Serial.println(walkBPM);
          char msg[50];
          sprintf(msg, "WalkBPM is %d\n", walkBPM);
      ...
      if (current_t > prev_t){
        ...
        // Here we need to calculate tapBPM
        prev_t = current_t;
        if (current_t - trialStartTime >= trialInterval+100){
          // Here we calculate tapBPM.
          tapBPM = round(60*1000/((responseArray[msg_number_array][2] - responseArray[1][2])/(msg_number_array - 1)));
          Serial.print("tapBPM: "); Serial.println(tapBPM);
          char msg[50];
          sprintf(msg, "tapBPM is %d\n", tapBPM);
          Serial.print("msg: "); Serial.println(msg);
      ...
      if (current_t > prev_t){
        if (trialStartTime == 0){
          ...
          totalNumberOfBeats = (walkBPM/60.0)*(trialInterval/1000.0); // This is the total number per Condition.
          ...
          metronome_interval = round(60E3/walkBPM);
      ...
      if (current_t > prev_t){
        if (trialStartTime == 0){
          trialStartTime = current_t;
          // Generate beat sequences
          totalNumberOfBeats = (tapBPM/60.0)*(trialInterval/1000.0); // This is the total number per Condition.
          totalNumberOfDeviants = 20*(trialInterval/1000/60); // 20 per minute
          Serial.print("totalNumberOfBeats: "); Serial.println(totalNumberOfBeats);
          Serial.print("totalNumberOfDeviants: "); Serial.println(totalNumberOfDeviants);
          setupOddball((int)totalNumberOfBeats, totalNumberOfDeviants);          
          nSnds = totalNumberOfBeats;
          metronome_interval = round(60E3/tapBPM);
          Serial.print("metronome_interval: "); Serial.println(metronome_interval);
          digitalWrite(greenLED, HIGH);
        }
        // Here is where we put the choices for each Condition
